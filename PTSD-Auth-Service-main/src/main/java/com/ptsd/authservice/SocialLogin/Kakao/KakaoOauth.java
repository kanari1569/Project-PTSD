package com.ptsd.authservice.SocialLogin.Kakao;

import java.util.HashMap;
import java.util.Map;
import java.util.stream.Collectors;

import com.ptsd.authservice.SocialLogin.SocialOauth;
...
//     private String KAKAO_DATA_ACCESS_SCOPE;
    ...
    @Value(("${OAuth2.kakao.userprofile-url}"))
    private String KAKAO_USERPROFILE_REQUEST_URL;
    ...
    public ResponseEntity<String> requestUserProfile(KakaoOauthToken oauthToken) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Authorization",oauthToken.getToken_type() + " " + oauthToken.getAccess_token());

        HttpEntity<MultiValueMap<String, String>> request = new HttpEntity(headers);
        ResponseEntity<String> response = restTemplate.exchange(KAKAO_USERPROFILE_REQUEST_URL, HttpMethod.GET,request,String.class);
        return response;
    }
    ...
    public KakaoProfile getUserProfile(ResponseEntity<String> userInfoRes) throws JsonProcessingException{
        KakaoProfile kakaoProfile =objectMapper.readValue(userInfoRes.getBody(),KakaoProfile.class);
        return kakaoProfile;
    }
}
